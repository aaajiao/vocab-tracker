...
| POST /sessions | id(UUID), language, mode=conversation/recall/cloze, topic, word_ids(UUID[]), target_minutes | 会话 |
...
只读令牌可以读取偏好和练习历史。`practice:write` 可写会话、事件和偏好，`sentences:write` 可保存句子。未练习/仅展示的词不提交事件；语音听不清不记作遗忘。练习开始的授权覆盖该次练习的合理记录，Skill 不逐词额外索要确认。
