...
- `useWords` — Core word CRUD. Account-owned cache-first loading (IndexedDB → authenticated learning API). Offline writes with sync. Filters, date grouping, stats.
- `useSentences` — Saved sentence management. Same cache-first + offline pattern as useWords.
- `useMaterialCollection` — Shared account isolation, pending-change projection, paginated refresh and stale-response guards for words/sentences.
...
- `wordsCache.ts` / `sentencesCache.ts` — Compatibility helpers over the account-owned material store.
- `materialStore.ts` / `materialQueue.ts` — Durable UUID-based requests, canonical records, receipts, aliases and per-owner synchronization. Preserve unowned legacy data as a recovery backup; never automatically upload it under the current account.
- `audioCache.ts` — IndexedDB for TTS audio blobs. Key format: `{language}_{text}`.
- `syncQueue.ts` — Coordinates material and review-event synchronization through the shared API. Temporary failures remain retryable; confirmed writes atomically update the material cache and remove the matching request.
...
- `SettingsDialog` / `SettingsPanel` — Accessible settings drawer, Codex connection, optional webpage AI, synchronization recovery and cache management.
- `Icons` — SVG icon collection. Speaker icon is state-aware (playing/cached indicators).

### Offline/PWA Data Flow

Cache-first architecture: load the current account's IndexedDB projection first, then refresh from the authenticated learning API. Background reads do not hide cached content.

Persist complete operations with stable UUIDs before sending. `useNetworkStatus` synchronizes on login, reconnect and its retry interval. Merge pending additions/deletions/updates with cloud snapshots. A lost response retries the same request; only acknowledged writes leave the queue. Failed saves keep the draft, explicit offline saves report pending status, and failed undo remains available. See `docs/app-reliability.md`.
...
- `words` — id, user_id, word, meaning, language ('en'|'de'), example, example_cn, category ('daily'|'professional'|'formal'|''), date, created_at, etymology. PK: id. FK: user_id → auth.users(id) (no ON DELETE CASCADE). No global UNIQUE constraint — the shared word API serializes normalized deduplication and stores request receipts; the stored word preserves its original casing (e.g. German nouns like "Haus").
...
All application tables enable RLS and revoke `anon` access. `words` and `saved_sentences` keep authenticated CRUD grants with owner policies (`saved_sentences` has no UPDATE policy). `review_states` grants authenticated SELECT only; mutations use the server event API. Learning API tables and private write receipt tables grant access only to `service_role`. This role bypasses RLS and must remain server-only.
