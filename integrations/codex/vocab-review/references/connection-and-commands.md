...
在 Vocab Tracker 网站登录，在 Codex 连接设置中创建个人访问令牌；选择 `vocabulary:read`，需要保存练习时加 `practice:write`，需要收藏句时加 `sentences:write`。
...
列表返回 `{data,meta}`；`meta.has_more` 和 `meta.next_offset` 用于下一页 `--offset`。复习项为 `{word,state}`。使用返回的真实 UUID，时区从偏好读取。云端尚未同步的离线新增词不会出现。
...
| 命令 | JSON 正文字段 |
|---|---|
| `start --json -` | `id?`, `language` (`en`/`de`), `mode` (`conversation`/`recall`/`cloze`), `topic`, `word_ids`, `target_minutes` |
...
CLI 在第一次发送前固定 UUID，作答自动加 `source: "codex"`、当前 ISO 时间和本机时区；通常应显式传偏好时区和实际作答时间。客户端不接受 `user_id`。服务端负责权限、所有权、排期和内容验证。

会话开启正文示例：

```json
{"language":"de","mode":"conversation","topic":"租房沟通","word_ids":["使用真实词UUID"],"target_minutes":10}
```
...
- 恢复练习用 `sessions --status active` 和 `resume` 返回的会话、已记录事件继续。不要因为 Codex 上下文丢失就把同一组已完成作答重新评分。

只读令牌不能 start/event/finish，但仍可通过 review、words、sentences 和历史展开练习。权限不足时报告未保存即可，不要求用户中断练习。
